...
static int get_exts(void) {
	...
	if (max_loaded_major < 3) {
		...
		if (num_exts_i > 0) {
			exts_i = (char**)sus_malloc((size_t)num_exts_i * (sizeof * exts_i));
		}
		...
		for (index = 0; index < num_exts_i; index++) {
			...
			size_t len = sus_strlen(gl_str_tmp);
...
static void free_exts(void) {
	if (exts_i != NULL) {
		int index;
		for (index = 0; index < num_exts_i; index++) {
			sus_free((char*)exts_i[index]);
		}
		sus_free((void*)exts_i);
		exts_i = NULL;
	}
}
...
static int has_ext(const char* ext) {
	...
	if (max_loaded_major < 3) {
		...
		for (index = 0; index < num_exts_i; index++) {
			...
			if (exts_i[index] != NULL && sus_strcmp(e, ext) == 0) {
				return 1;
			}
		}
	}
...
static void find_coreGL(void) {
	...
	sus_parsingA(version, "%d.%d", &major, &minor);
